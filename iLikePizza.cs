...
public class RobotC : MonoBehaviour
{
    ...
    void Wavefront(Positions goal)
    {
        if ((goal.position.x != posx) || (goal.position.y != posy))
        {
            ...
            Debug.Log(posx+" "+posy+" "+ goal.position.x+" "+ goal.position.y);
            //trovo il percorso
            while (!isGoalReached)
            {
                i = 0;
    ...
    void Start()
    {
        ...
        listPos = new Positions[55];
        newPos = new CellPos[35];
        oldPos = new CellPos[35];
        journeyBack = new CellPos[50];
        journey = new Way[50];
    ...
    void FindClosestPosition(){
        //Wavefront fino a quando trovo la posizione da visitare più vicina
        //Controllo che non sia già arrivato
        if ((listPos[(n_listPos - 1)].position.x != posx) || (listPos[(n_listPos - 1)].position.y != posy))
        {
            ...
            lengthListNewPos = 0;
            CellPos lastPos;
            lastPos.x = osToReach.x;
            lastPos.y = osToReach.y;
            newPos[lengthListNewPos].x = lastPos.x;
            newPos[lengthListNewPos].y = lastPos.y;
            lengthListNewPos++;
            while (!wayFound)
            ...
            if (n_listPos > 0)
            {
                ...
                if (!AlreadyVisited(listPos[(n_listPos - 1)]))
                {
                    ...
                    if (goToPos)
                    {
                        int valx;
                        int valy;
                        Debug.Log(journeyBack[lengthJourneyBack-1].x + " " + journeyBack[lengthJourneyBack-1].y);
